...
export default function UseCallbackTest() {
  ...
  const memoizationMeowCallback = useCallback(() => {
    setMeowCount((n) => n + 1);
  }, []);

  const memoizationBarkedCallback = useCallback(() => {
  ...
  return (
    <div>
      ...
      <MeowButton onClick={memoizationMeowCallback} />
      <BarkButton onClick={memoizationBarkedCallback} />
    </div>
  );
}
