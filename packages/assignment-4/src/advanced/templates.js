...
export const MainLayout = ({ items }) => `<div class="bg-gray-100 p-8">
    <div class="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl p-8">
      <h1 class="text-2xl font-bold mb-4">장바구니</h1>
        ...
        ${items.reduce((acc, cur) => acc + ProductOption(cur), '')}
      </select>
...
export const CartTotal = ({ total, discountRate }) =>
  `<div id="cart-total" class="text-xl font-bold my-4">총액: ${total}원${discountRate ? `<span class="text-green-500 ml-2">(${(discountRate * 100).toFixed(1)}% 할인 적용)</span>` : ''}</div>`;
